...
bool segmentCollisionSimple(vector2f v11, vector2f v12, vector2f v21, vector2f v22, vector2f c1, vector2f c2)
{
    bool flag1 = (abs(c1.x - c2.x)<=30);
    bool flag2 = (abs(c1.y - c2.y)<=30);
    if (flag1 and flag2) {
        return true;
    }
    return false;
...
void solveElasticCollision(vector2f norm, Collider* source, Collider* obj2){
    //std::cout << "solveElasticCollision!" << std::endl;
	float module = sqrt(pow(norm.x, 2) + pow(norm.y, 2));
  	norm.x = norm.x/module;
  	norm.y = norm.y/module;
  	vector2f tau = {-norm.y, norm.x};
...
void Collider::elasticCollision(Collider* source, Collider* obj2){
    ...
    if (obj2->obj->name.compare("cristal") == 0) {
        obj2->obj->x=rand() % 10;
        obj2->obj->y=-200;
        obj2->obj->life+=1;
    }
...
bool Collider::isCollided(Collider* sample){
	...
	for (int i = 0; i < this->phys_model.getPointCount() - 1; i++){
        for (int j = 0; j < sample->phys_model.getPointCount() - 1; j++){
			...
			std::string name1 = this->obj->name;
			std::string name2 = sample->obj->name;

			if (name1.compare("player") == 0 ) {
                if ( name2.compare("cristal") == 0 ) {
                    if (segmentCollisionSimple(point_1i, point_1iplus, point_2j, point_2jplus, c1, c2)) {
                        return true;
                    }

                } else if ( name2.compare("enemy") == 0 ) {
                    if (segmentCollisionSimple(point_1i, point_1iplus, point_2j, point_2jplus, c1, c2)) {
                        sample->obj->life+=1;
                        sample->obj->x= 200;
                        sample->obj->y= -200;
